	...
	u64 addr, u64 length, bool null_err)
{
	...
	if (null_err)
		pr_err("%s addr %llx length %llx not found!\n", __func__, addr, length);
	else
	...
	unsigned char cmd, u64 addr, u64 data, u32 mask, u32 gpr, u32 dma_offset, u32 dma_size)
{
	...
	case CMD_READ:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_err("[VCU] CMD_READ wrong addr: 0x%llx\n", addr);
	break;
	case CMD_WRITE:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			cmdq_pkt_write(pkt, vcu->clt_base, addr, data, mask);
		else
			pr_err("[VCU] CMD_WRITE wrong addr: 0x%llx 0x%llx 0x%x\n",
				addr, data, mask);
	break;
	...
	case CMD_POLL_REG:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_err("[VCU] CMD_POLL_REG wrong addr: 0x%llx 0x%llx 0x%x\n",
				addr, data, mask);
	break;
	case CMD_WAIT_EVENT:
	...
	case CMD_MEM_MV:
		...
		if ((reg_check == 0 || src_page != NULL) && dst_page != NULL) {
			...
			pr_err("[VCU] CMD_MEM_MV wrong addr/data: 0x%llx 0x%llx\n",
				addr, data);
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	if (ret < 0) {
		pr_err("[VCU] cmdq flush fail pkt %p\n", pkt_ptr);
		vcu_gce_release_used_pages(&vcu->gce_info[j].used_pages[gce_order]);
	}
...
void vcu_get_gce_lock(struct platform_device *pdev, unsigned long codec_type)
{
	...
	if (pdev == NULL) {
		pr_err("[VCU] %s platform device is null.\n", __func__);
		return;
	}
	if (codec_type >= VCU_CODEC_MAX) {
		pr_err("[VCU] %s invalid codec type %ld.\n", __func__, codec_type);
		return;
	}
	vcu = platform_get_drvdata(pdev);
...
static int mtk_vcu_open(struct inode *inode, struct file *file)
{
	...
	pr_debug("[VCU] %s name: %s pid %d tgid %d open_cnt %d current %p group_leader %p\n",
